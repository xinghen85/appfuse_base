...
public class JavascriptValidatorTag extends BodyTagSupport {
    ...
    private String ignoreHidden;
    
    

    public String getIgnoreHidden() {
		return ignoreHidden;
	}

	public void setIgnoreHidden(String ignoreHidden) {
	...
	public void setMethodCheck(String methodCheck) {
    ...
    public String getNewValidateTag(String formName){
    	Map<String,List<String>> methodCheckMap=new HashMap<String,List<String>>();
    	if(methodCheck!=null && !"".equals(methodCheck)){
    		String[] mca0=methodCheck.split(";");
    		if(mca0!=null && mca0.length>0){
    			for(String str0:mca0){
    				if(str0!=null && !"".equals(str0)){
    		    		String[] mca1=str0.split(":");
    		    		if(mca1!=null && mca1.length>=2){
    		    			MapUtil.appendListEntityToMap(methodCheckMap, mca1[0], mca1[1]+":true");
    		    		}
    		    	}
    			}
    		}
    	...
    	ExtendFormInfo cfgFormInfo=CfgFormInfoCache.getInstance().getCfgFormInfoByFormCode(formName);
    	if(cfgFormInfo!=null){
    		List<AnnoInfo> annos=cfgFormInfo.getFieldAnnoInfoList();
        	...
        	if(annos!=null && annos.size()>0){
    	    	for(AnnoInfo one:annos){
    	    		...
    	    		if(methodCheckMap.containsKey(one.getField().getName())){
    	    			validates.addAll(methodCheckMap.get(one.getField().getName()));
    	    		}
    	    		...
    	    		if(validates.size()>0){
    	    			if(one.getType()==3){
    	    				rules.add("\""+one.getField().getName()+".id\":{"+ListUtil.toString(validates, ",")+"}");
    	    			}else{
    	...
    	if(flist!=null){
    		for(CfgCustomProperty ccp:flist){
            	...
            	if(validates.size()>0){
        			rules.add("\"customPropertyMap['"+ccp.getPropertyCode()+"']\":{"+ListUtil.toString(validates, ",")+"}");
        		}
            }
    	}
        
    	StringBuffer sb=new StringBuffer();
    	sb.append("var validate_"+formName+"Form=$(\"#"+formName+"Form\").validate({\r\n");
    	sb.append("	rules:{\r\n");
    	sb.append("			"+ListUtil.toString(rules, ",			\r\n")+"\r\n");
    	sb.append("	},\r\n");
    	if("true".equals(this.ignoreHidden)){
    		sb.append("	ignore:\"\",\r\n");
    	}
    	
    	sb.append("	onkeyup:false,\r\n");
    	sb.append("	onclick:false,\r\n");
    ...
    public int doStartTag() throws JspException {
        ...
        if (formName != null) {
            ...
            if ("true".equalsIgnoreCase(dynamicJavascript)) {
                ...
                results.append(getNewValidateTag(formName));
